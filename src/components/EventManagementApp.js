...
const EventManagementApp = (props) => {
  const events = [
    ...
    { title: "LARPing with Nick", timePeriod: "4:00pm - 5:00pm", description: "Don't get injured", eventType: "business" }
  ]
  // debugger

  const eventTilesArray = events.map((eventObject) => {
    // console.log(eventObject);
    return (
      <EventTile 
        key={eventObject.title}
  ...
  return (
    <div className="event-management-app">
      ...
      {/* <EventTile eventObject={events[0]} />
      <EventTile eventObject={events[1]} />
      <EventTile eventObject={events[2]} />
      <EventTile eventObject={events[3]} /> */}
